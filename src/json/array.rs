...
///     set.encode(&mut buf);
///
///     assert_eq!(matches!(serde_json::from_str::<HashSet<String>>(set.stringify().as_str()), Ok(set)), true);
///     assert_eq!(matches!(serde_json::from_slice::<HashSet<String>>(buf.as_slice()), Ok(set)), true);
/// }
///
/// {
///     let mut buf = vec![];
